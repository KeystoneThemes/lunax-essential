...
(function ($) {
   ...
   */
  const LunaxVideoTestimonial = function ($scope, $) {
    const slider = $($('.lunax_vtm_slider', $scope)[0]);
    ...
    new elementorFrontend.utils.swiper(slider, sliderSettings).then(newSwiperInstance => newSwiperInstance).then((newSwiperInstance)=>{
      ...
      const tsm_video = $('.lunax_vtm_slider .video', $scope);
      const play_video = $('.lunax_vtm_slider .play-video', $scope);
  ...
  $(window).on('elementor/frontend/init', function () {
    elementorFrontend.hooks.addAction('frontend/element_ready/lunax--video-testimonial.default', LunaxVideoTestimonial);
  });
})(jQuery);
