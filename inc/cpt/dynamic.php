...
class Wdb_Dynamic_Cpt extends Custom_Post
{
    ...
    function isearch_filter_post_type($query) {
        ...
        if ( ! is_admin() && $query->is_main_query() ) {
            ...
            $search_result_post_types = lunax_option('search_result_post_types', false);            
    ...
    public function create_taxonomy(){
        ...
        $cpt_options = get_option(LUNAX_OPTION_KEY);
        if( isset( $cpt_options[ 'cpt_taxonomy_options' ] ) && is_array($cpt_options[ 'cpt_taxonomy_options' ]) && count($cpt_options[ 'cpt_taxonomy_options' ]) > 0){
            ...
            foreach($options as $option){
                ...
                if($option['taxonomy_name']  != '' && isset($option['post_types'])){
                    ...
                    $labels = array(
                        'name'              => _x( $option['taxonomy_plural_label'], $option['taxonomy_plural_label'], 'lunax-essential' ),
                        'singular_name'     => _x( $option['taxonomy_label'], $option['taxonomy_label'] , 'lunax-essential' ),
                        'search_items'      => __( "Search $plural_label", 'lunax-essential' ),
                        'all_items'         => __( "All $plural_label", 'lunax-essential' ),
                        'parent_item'       => __( "Parent $singular_label ", 'lunax-essential' ),
                        'parent_item_colon' => __( "Parent $singular_label :", 'lunax-essential' ),
                        'edit_item'         => __( "Edit $singular_label", 'lunax-essential' ),
                        'update_item'       => __( "Update $singular_label ", 'lunax-essential' ),
                        'add_new_item'      => __( "Add New $singular_label ", 'lunax-essential' ),
                        'new_item_name'     => __( "New $singular_label ", 'lunax-essential' ),
                        'menu_name'         => __( $singular_label , 'lunax-essential' ),
                    );
