 ...
 * recent post widget
 */
class Lunax_Recent_Post extends WP_Widget {

	function __construct() {

		$widget_opt = array(
			'classname'		 => 'joya-widget blog-box blog-post',
			'description'	 => esc_html__('Info Recent post with thumbnail','lunax-essential')
		);

		parent::__construct( 'joya-recent-post', esc_html__( 'Lunax recent post', 'lunax-essential' ), $widget_opt );
	}
	...
	function widget( $args, $instance ) {
        ...
        if ( !empty( $instance[ 'lunax_post_type' ] ) ) {
			$lunax_post_type = $instance[ 'lunax_post_type' ];
		} else {
			$lunax_post_type = 'recent';
		} 
								...
								<div class="default-sidebar__recent-item">
									<?php
										...
										if($image_show !='' && isset($thumbnail[0])): 
											$img = $thumbnail[ 0 ];
										    echo '<a class="d-block mb-10" href="'.get_the_permalink().'"><img src="' . esc_url( $img ) . '" alt="' . esc_attr__('thumb','lunax-essential') . '"></a>';
										endif;
	...
	function update( $new_instance, $old_instance ) {
		...
		$old_instance[ 'lunax_post_type' ] = $new_instance[ 'lunax_post_type' ];
	...
	function form( $instance ) {
        ...
        if ( isset( $instance[ 'lunax_post_type' ] ) ) {
             $lunax_post_type = $instance['lunax_post_type'];  
        }else{
             $lunax_post_type = 'recent';  
        } 
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = esc_html__( 'Recent posts', 'lunax-essential' );
		}
        ...
        <p>
		    <select class='widefat' id="<?php echo $this->get_field_id('lunax_post_type'); ?>"
					name="<?php echo $this->get_field_name('lunax_post_type'); ?>" type="text">
					<option
						value='recent' 
						<?php echo ($lunax_post_type=='recent')?'selected':''; ?>>
						<?php echo esc_html__('Recent post','lunax-essential'); ?>
					</option>
					<option 
						value='populer'
						<?php echo ($lunax_post_type=='populer')?'selected':''; ?>>
						<?php echo esc_html__('Populer post','lunax-essential'); ?>
					</option> 
			</select>                
        </p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php esc_html_e( 'Number of posts:', 'lunax-essential' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts_title_limit' ) ); ?>"><?php esc_html_e( 'Title limit:', 'lunax-essential' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts_excerp_limit' ) ); ?>"><?php esc_html_e( 'Content limit:', 'lunax-essential' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts_date_format' ) ); ?>"><?php esc_html_e( 'Date Format:', 'lunax-essential' ); ?></label>
