...
class Lunax_Essentail_Admin
{
    ...
    add_action( 'wp_ajax_wdb_admin_get_cache_cpt', [$this,'lunax_admin_get_custom_post_types'] );
	}
	...
	function lunax_admin_get_custom_post_types(){
	
		if ( !wp_verify_nonce( $_REQUEST['nonce'], "wdb_theme_secure")) {
			exit("No naughty business please");
		}
		lunax_get_cache_tax_types();
		lunax_get_cache_post_types();
	    $array_result = array(
            'status' => esc_html__('Cache post type and taxonomy','lunax-essential'),
        ); 
