...
Class Lunax_Optimize_Assets{
 ...
 public function __construct(){
    ...
    // contact form 7
    if( lunax_option('ondemand_contact_form_7', true ) ){
    ...
    if(lunax_option( 'defer_js_and_css' , false) ){
 ...
 public function enqueue_scripts(){    
    if(!defined('LUNAX_CSS')){
       return;
    }
    wp_register_style( 'lunax-custom-icons' , LUNAX_CSS . '/custom-icons.min.css', null, LUNAX_ESSENTIAL_VERSION );
    wp_enqueue_style( 'lunax-custom-icons' );  
 }
 ...
 function info_optimize_theme_css($post_id)
  {
    ...
    if(!lunax_option('optimize_asset_enable')){
        return;
    }
    ...
    $deque               = [];
    $css_content         = '';
    $optimize_minify_css = lunax_option('optimize_asset_enable');
    ...
    lunax_new_directory($this->directory_path);
    foreach( $wp_styles->queue as $style ) {
        if( lunax_has_string_prefix( $style , $prefix ) && isset( $wp_styles->registered[ $style ] ) ){   
          $deque[$style] = $wp_styles->registered[$style]->src;             
        }             
    }
    
    foreach( $wp_styles->registered as $key => $style ) {          
        if( lunax_has_string_prefix( $key , $prefix ) ){                 
            $deque[$key] = $style->src;
        }             
    }
    ...
    if( $css_content !='' ){
        if($optimize_minify_css && 1==2){
            $wp_filesystem->put_contents($dir_path , lunax_minify_CSS( $css_content ) , 755 );
        }else{
  ...
  function enqueue_pure_styles() { 
    ...
    if( file_exists( $dirpath ) ){       
        ...
        foreach( $wp_styles->queue as $style ) { 
            ...
            if( lunax_has_string_prefix( $style , $prefix ) ){   
        ...
        wp_enqueue_style('lunax-styles-'.get_queried_object_id(), $path);
    }
