<?php 
  ...
  $json_data = lunax_get_config_value_by_name('elementor/archive');
  $args = array(
    'numberposts' => 15,
  ...
  $_hf_html = sprintf( '<a href="%s" target="_blank">%s</a>' , esc_url($hf_page), esc_html__('There is no layout manage From Builder Page','lunax-essential') );
?>
<style>
...
<div class="wdb-modal wdb--remote-layouts">
  ...
  <div class="wdb-modal__inner">
    ...
    <h2><?php echo esc_html__('Blog Template Layout Import','lunax-essential') ?></h2>
      <div class="wdb-content-install">
      <div class="wdb-single-tpl-wrapper">
            <h4><?php // echo esc_html__('Elementor Layout','lunax-essential') ?></h4>
            <h1></h1>
            ...
            <?php foreach($json_data as $item){ ?> 
                <div class="wdb-image-tpl <?php //echo $active_id == $item->ID ? 'active' : ''; ?>">
                    ...
                    <div class="wdb-hover-element">
                        <span data-id="<?php echo $item['uniq_id']; ?>" class="wdb--blog-tpl-install-remote"><?php echo esc_html__('Activate','lunax-essential'); ?></span>
                    </div>
                </div>
                <?php } ?>
