...
abstract class lunax_MinificationSequenceFinder
{
    public $start_idx;
    public $end_idx;
    public $type;
...
class lunax_StringSequenceFinder extends lunax_MinificationSequenceFinder
{
...
class lunax_QuoteSequenceFinder extends lunax_MinificationSequenceFinder
{
...
$singlelunax_QuoteSequenceFinder = new lunax_QuoteSequenceFinder('\'');
$doublelunax_QuoteSequenceFinder = new lunax_QuoteSequenceFinder('"');
$blockCommentFinder = new lunax_StringSequenceFinder('/*', '*/');
$minificationStore = array();

function lunax_getNextMinificationPlaceholder(){
    global $minificationStore;
    return '<-!!-' . sizeof($minificationStore) . '-!!->';
}
function lunax_getNextSpecialSequence($string, $sequences){
...
function lunax_minify_CSS($css){
    global $minificationStore, $singlelunax_QuoteSequenceFinder, $doublelunax_QuoteSequenceFinder, $blockCommentFinder;
    $css_special_chars = array($blockCommentFinder, // CSS Comment
    $singlelunax_QuoteSequenceFinder, // single quote escape, e.g. :before{ content: '-';}
    $doublelunax_QuoteSequenceFinder); // double quote
    // pull out everything that needs to be pulled out and saved
    while ($sequence = lunax_getNextSpecialSequence($css, $css_special_chars)){
        switch ($sequence->type){
        case '/*': // remove comments
        $css = substr($css, 0, $sequence->start_idx) . substr($css, $sequence->end_idx);
        break;
        default: // strings that need to be preservered
        $placeholder = lunax_getNextMinificationPlaceholder();
