...
* @version 1.0 
*/
function LUNAX_ESSENTIAL_theme_option_footer_code() {
   $html = lunax_option('opt-tabbed-code');
   // html
...
add_action( 'wp_footer', 'LUNAX_ESSENTIAL_theme_option_footer_code' );

function LUNAX_ESSENTIAL_theme_option_page_footer_code() {

   if(is_page()) {
      $html = lunax_meta_option(get_the_id(),'opt-tabbed-code',false);      
      // html
      if(is_array($html) && array_key_exists('opt_code_editor_html',$html) && $html['opt_code_editor_html'] !=''){
         libxml_use_internal_errors(true);
         $dom = New DOMDocument();
...
add_action( 'wp_footer', 'LUNAX_ESSENTIAL_theme_option_page_footer_code' );
   ...
   if ( ( !is_wp_error($response)) && (200 === wp_remote_retrieve_response_code( $response ) ) ) {
		...
		if( json_last_error() === JSON_ERROR_NONE ) {
         ...
         if(version_compare($theme_data->get( 'Version' ), $responseBody->data->new_version, '<')){
            ...
            wp_send_json_error( ['msg' => esc_html__('Update not available','lunax-essential') ] );
         }
...
function LUNAX_ESSENTIAL_theme_option_header_code() {
   $html        = lunax_option('opt-tabbed-code');
   $tab_size    = '991.98';
   $mobile_size = '767.98';
   
   ?>
      <style id="lunax-theme-global-css">
        <?php 
...
add_action('wp_head', 'LUNAX_ESSENTIAL_theme_option_header_code');

function LUNAX_ESSENTIAL_theme_option_page_header_code() {
   $html        = lunax_meta_option(get_the_id(),'opt-tabbed-code', false);
   $tab_size    = '991.98';
   $mobile_size = '767.98';
   if(!$html){
...
add_action('wp_head', 'LUNAX_ESSENTIAL_theme_option_page_header_code');

function lunax_script_custom_data($data){

   if(lunax_option('offcanvas_responsive_enable',0)){
     $data['offcanvas_responsive_enable'] = true;
     $data['offcanvas_responsive_menu_width'] = lunax_option('offcanvas_responsive_menu_width');  
   }
   
   if(lunax_option('offcanvas_menu_icon_plus') && lunax_option('offcanvas_menu_icon_plus') !=''){
      $data['offcanvas_menu_icon_plus'] = sprintf('<i class="%s"></i>',lunax_option('offcanvas_menu_icon_plus')); 
   }
   
   if(lunax_option('offcanvas_menu_icon_minus') && lunax_option('offcanvas_menu_icon_minus') !=''){
      $data['offcanvas_menu_icon_minus'] = sprintf('<i class="%s"></i>',lunax_option('offcanvas_menu_icon_minus')); 
   }
   
   if( lunax_option('sticky_header',0) ){
      $data['sticky_enable'] = true;
      $data['sticky_header_top'] = lunax_option('sticky_header_start_from',150);
   }

   return $data;
}

add_filter('lunax/script/custom/data' , 'lunax_script_custom_data');


//add_action('elementor/page_templates/canvas/before_content','lunax_esssen_preloader_template_part');

function lunax_esssen_preloader_template_part(){
...
function wdb_body_open_enable_page_background(){
   ...
   $bg_enable         = false;
   $global_settings   = lunax_option('general_full_site_background');
   $background_preset = lunax_option('general_fullsite_background_preset');
   $custom_background = lunax_option('general_full_site_custom_background');
   $url               = '';
   ...
   if( $global_settings ){   
      if( $background_preset == 'custom' && isset( $custom_background[ 'url' ] ) ){
        ...
        if( $the_url       = LUNAX_ESSENTIAL_get_background_patterns( $background_preset ) ){
            $url           = $the_url;
            $bg_enable     = true;
        }
      }      
   ...
   if(is_page()){
      $global_settings   = lunax_meta_option( get_the_id() , 'general_full_site_background' );
      $background_preset = lunax_meta_option( get_the_id() , 'general_fullsite_background_preset' );
      $custom_background = lunax_meta_option( get_the_id() , 'general_full_site_custom_background' );
...
add_action('elementor/widgets/register', function($widget_manager){
      if(lunax_theme_service_pass()){ return;}       
	...
	$widget_manager->unregister_widget_type('lunax-service');
...
add_filter( 'wp_check_filetype_and_ext', function($data, $file, $filename, $mimes) {
 ...
 function LUNAX_ESSENTIAL_crw_mime_types( $mimes ){
   $mimes['svg'] = 'image/svg+xml';
   return $mimes;
 }
 add_filter( 'upload_mimes', 'LUNAX_ESSENTIAL_crw_mime_types' );
 
 function LUNAX_ESSENTIAL_crw_fix_svg() {
 ...
 add_action( 'admin_head', 'LUNAX_ESSENTIAL_crw_fix_svg' );

add_action( 'wp_body_open', function () {
	if ( lunax_option( 'enable_scroll_top', 1 ) ) { ?>
