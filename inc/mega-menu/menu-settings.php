...
Class Wdb_Mega_Menu_Settings{
    ...
    function _admin_footer() {
        ...
        if(isset($current_screen->base) && $current_screen->base == 'nav-menus'){         
            ...
            esc_html__('Mega Menu Templates', 'lunax-essential'),
            $this->get_latest_tpl_html(),
            esc_html__('Post Title','lunax-essential'),
            esc_html__('Type mega menu section','lunax-essential'),
            esc_html__('Submit','lunax-essential')
            );        
        }
    }
    ...
    public function admin_enqueue_scripts(){    
       ...
       wp_localize_script( 'wdb-mega-menu-admin', 'lunax_obj', $lunax_data);
    }
    ...
    public function menu_item_settings( $item_id, $item , $depth ){
        ...
        $text_popup = is_numeric( $menu_item_ele_id ) ? esc_html__(' - Settings', 'lunax-essential') : esc_html__('Add New', 'lunax-essential');
        if($depth === 0){
        ?>
        <div class="wdb--mega-menu-switcher">
            <div class="wdb--menu-flex-container wdb--menu-setting-header-js">
                <h4><?php echo esc_html__('Mega Menu' , 'lunax-essential'); ?></h4>
                <div class="wdb--mega-menu-additional-option" style="<?php echo esc_attr($menu_item_enable == 'on' ? '' : 'display:none;'); ?>">
                    <?php if(is_numeric($menu_item_ele_id)){ ?>
                        <div class="wdb--mega-menu--fullwidth">
                            ...
                            <label><?php echo esc_html__('Fullwidth','lunax-essential'); ?></label>
                        </div>
                        <a target="_blank" href="<?php echo esc_url(get_edit_post_link($menu_item_ele_id)); ?>" class="button-secondary"><?php echo esc_html__('Edit', 'lunax-essential') ?></a>
                    <?php } ?>
                </div>
            </div>
            <div class="wdb--menu-flex-container">
                ...
                <div class="item wdb-width--100 _menu_item_wdb_mega_menu_tpl_id" <?php echo $menu_item_enable =='on' ? '' : 'hidden'; ?>> 
                    ...
                    <input type="hidden" value="<?php echo $menu_item_ele_id; ?>"  name="_menu_item_wdb_mega_menu_tpl_id[<?php echo $item_id ;?>]" placeholder="<?php echo esc_html__('Select template from here','lunax-essential'); ?>" class="wdb--mega-menu-search-fld wdb-width--100"  />
                </div>
            </div>
        </div>
        <?php
