...
<div class="pd-header layout-2"> 
    <?php if($settings['custom_mobile_image']['url'] !=''){ ?>
			<a href="<?php echo esc_url(home_url('/')); ?>">
				<img src="<?php echo esc_url($settings['custom_mobile_image']['url']); ?>" class="show-dark" alt="<?php echo esc_attr__('Site Logo','lunax-essential') ?>">          
      </a>
    <?php } ?>
    <button class="header__navicon-2">             
        <?php if($bar !=''){ ?>
		        <img id="header_navicon_2" class="icon" src="<?php echo esc_url($bar); ?>" alt="<?php echo esc_html__('custom bar','lunax-essential'); ?>"/>
		    <?php } ?>
    </button>
</div>
<<?php echo esc_attr($settings['header_size']); ?> class="wdb-header-pin-js layout-2 header__area-2 wdb-landing-page-padding <?php echo esc_attr($settings['mobile_direction']); ?> ">
    <div class="header__inner-2" data-urlattr="<?php echo esc_attr($settings['enable_url_attr']); ?>">
      <?php $close_icon = lunax_render_elementor_icons($settings['close_icon']); ?>
      <button class="header__navicon-2">
      ...
      <div class="header__logo-2 resume-header__logo">
        <a href="<?php echo esc_url(home_url('/')); ?>">
			    <img src="<?php echo esc_url($site_image); ?>" class="show-dark" alt="<?php echo esc_attr__('Site Logo','lunax-essential') ?>">          
        </a>
      </div>           
      <?php } ?>
      ...
      $search_icon = lunax_render_elementor_icons($settings['search_icon']);
      ?>
      <div class="header__search-2">
        <form method="get" class="search-form" action="<?php echo home_url( '/' ); ?>">
          <input type="text" placeholder="search" value="<?php echo get_search_query() ?>" name="s">
          <button class="icon-search" aria-label="<?php echo esc_attr__('Search Button', 'lunax-essential'); ?>">
            <?php if($search_icon =='' ){ ?>
      ...
      $cta_icon = lunax_render_elementor_icons($settings['cta_icon']);
      ?>
