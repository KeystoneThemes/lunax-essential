  ...
  <div class="open-offcanvas wdb--info-animated-offcanvas" data-content_source="<?php echo esc_attr($settings['content_source']); ?>" data-preset="<?php echo esc_attr($preset_style); ?>">
        <?php if( $settings['menu_button_text'] == '' ){ ?>
		                ...
		                <?php echo lunax_get_attachment_image_html( $settings, 'thumbnail', 'sticky_bar', ['class' => 'wdb-sticky-bar'] ); ?>
		            <?php }else{ ?>
      ...
      <div class="offcanvas-4__button-wrapper offcanvas-close__button-wrapper offcanvas--close--button-js">
        <button class="text-close-button">       
           <?php if($settings['default_close_contentss'] =='yes') { ?>
                <div class="bars">
                  <?php echo esc_html__( 'Close' , 'lunax-essential' ); ?>
                  <span></span>
                  <span></span>
                </div>
					      <?php }else{ ?>
                  <?php echo esc_html( $settings[ 'close_text' ] ); ?>
                  <?php if(lunax_render_elementor_icons($settings['close_icon'])){ ?>
  				          <div class="off-close-icon">
  						        <?php echo \LunaxEssentialApp\Inc\Iinherit\Wdb_Icon_Manager::render_icon( $settings['close_icon'], [ 'aria-hidden' => 'true' ] );
  						        ?>
  				          </div>
  					      <?php } ?>
					      <?php } ?>
        ...
        <?php if(!empty($settings['follow_info'])){ ?>
          <div class="offcanvas-4__social d-none d-md-flex">
            <div class="offcanvas-4__social-links offcanvas__social__links">
              ...
              ?>
                <a class="elementor-repeater-item-<?php echo esc_attr( $fol['_id'] ); ?>" <?php echo $this->get_render_attribute_string( 'follow_me_link' ); ?>><?php echo \LunaxEssentialApp\Inc\Iinherit\Wdb_Icon_Manager::render_icon( $fol['icon'] , [ 'aria-hidden' => 'true' ] ); ?></a>
              <?php } ?>              
            </div>
