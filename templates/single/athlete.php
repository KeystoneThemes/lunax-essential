 ...
 */
  ...
  get_header();   
  $option = lunax_option( 'blog_post_preset_grp' );   
  ...
  $preset_blog_view             = isset($option['preset_blog_view']) ? $option['preset_blog_view'] : 1;    
	$has_social_share             = lunax_social_share();
	$general_social_share         = lunax_social_share();   
?>
<style>
            ...
            <div class="athletic__blogdetails-wrapper">
              ...
              <div class="athletic__blogdetails-right pt-100">
                <ul class="athletic__blogdetails-meta">
                  <li>
                    <?php                   
                      if(lunax_option('blog_author',0)){
                        echo esc_html__('by','lunax-essential') .' '. get_the_author(); 
                      }
                    ?>
                  </li>
                  <?php if(lunax_option('blog_date','1')){ ?>
                  <li><?php echo get_the_date(get_option( 'date_format' )); ?></li>
                  <?php } ?>
                  <?php if($preset_blog_view){ ?>
                  <li><?php echo get_post_meta(get_the_id(),'lunax_post_views_count',true) ?> <?php echo esc_html__('Views','lunax-essential'); ?></li>
                  <?php } ?>
                </ul>
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class(['blog-details__fullBody']); ?>>
                  <?php lunax_get_template('content-single.php',[], LUNAX_ESSENTIAL_DIR_PATH , LUNAX_ESSENTIAL_DIR_PATH . '/templates/single/'); ?>
                </article>
                <?php if($general_social_share){ ?>
                  <div class="social-share__wrapper mt-40">
                    <h2 class="social-share__title">
                      <i class="fa-solid fa-share-nodes"></i> <?php echo esc_html__('Shares','lunax-essential') ?>
                    </h2>
